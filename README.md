...
## HealthKit and Locally Stored Hiking Data

HealthKit-derived information—including workouts, dates, routes, coordinates, distance, duration, elevation, energy, heart-rate information, and weather recorded with a workout—is stored in the app’s local data store on your device.

My Hiking Library:

- Does not write or modify information in Apple Health
- Does not store personal HealthKit information in iCloud
...
When you open an eligible completed hike, My Hiking Library may send that hike’s trail coordinate and representative date and time directly to Apple Weather through Apple’s WeatherKit service. This is used only to retrieve historical conditions associated with that hike, such as temperature, humidity, precipitation, wind, and general weather conditions.

This request does not use or transmit your current live location. The developer does not receive the request or the returned weather information.

Weather returned by Apple Weather is temporarily cached in memory to avoid unnecessary repeat requests during the current app session. It is not permanently stored by the app and is not synchronized through iCloud.

Apple’s handling of information submitted to Apple services is governed by the [Apple Privacy Policy](https://www.apple.com/legal/privacy/).

## Current Location

Current-location access is used only to show your position when using supported map features.
...
The app may store settings and operational information on your device, including:
...
- Use HealthKit, Photo Library, weather, or location information for advertising or marketing
...
- **Apple Health:** Manage My Hiking Library’s access from the Health app.
- **Photos:** Manage Photo Library access in iOS Settings.
- **Location:** Manage Location access in iOS Settings.

Revoking permission prevents future access to the corresponding protected information. Previously cached local app information may remain until the app’s local data is deleted or the app is uninstalled.
...
My Hiking Library is not directed toward children and is not designed to knowingly collect personal information from children through developer-operated services. The developer does not operate an account system, analytics service, advertising service, or user-data server for the app.

## Changes to This Policy

This policy may be updated when the app’s features or data practices change. The effective date at the top of this page identifies the latest revision.

Material changes will be reflected in the published policy before or when the corresponding app update becomes available.
